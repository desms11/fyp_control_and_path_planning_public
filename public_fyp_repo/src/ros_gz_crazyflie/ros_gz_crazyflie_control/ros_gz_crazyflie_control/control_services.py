...
class ControlServices(Node):
    ...
    def __init__(self):
        ...
        auto_takeoff = self.get_parameter('auto_takeoff').value
    ...
    def timer_callback(self):
        ...
        # One-shot auto takeoff at startup when enabled.
        if self.auto_takeoff and not self.auto_takeoff_done and not self.is_flying:
            height_command = max(height_command, 0.5)

        # If not flying and receiving a positive height command, take off.
